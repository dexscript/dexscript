...
public class DexGreaterThanExpr extends DexBinaryOperator implements DexInvocationExpr {

    private static final int LEFT_RANK = 10;
    private static final int RIGHT_RANK = 10;
    private DexInvocation invocation;

    public DexGreaterThanExpr(Text src, DexExpr left) {
        super(src, left);
    ...
    public DexInvocation invocation() {
        if (invocation == null) {
            invocation = new DexInvocation(pkg(), "GreaterThan__", left(), right());
        }
        return invocation;
    }
}