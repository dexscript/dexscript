...
public class DexParser implements PsiParser, LightPsiParser {
  ...
  /* ********************************************************** */
  // !('!' | '!=' | '%' | '%=' | '&&' | '&' | '&=' | '&^' | '&^=' | '(' | ')' | '*' | '*=' | '+' | '++' | '+=' | ',' | '-' | '--' | '-=' | '...' | '/' | '/=' | ':' | ';' | '<' | '<-' | '<<' | '<<=' | '<=' | '<NL>' | '=' | '==' | '>' | '>=' | '>>' | '>>=' | '[' | ']' | '^' | '^=' | 'type' | '{' | '|' | '|=' | '||' | '}' | break | case | chan | char | const | continue | decimali | default | defer | else | fallthrough | float | floati | for | func | go | goto | hex | identifier | if | int | interface | map | oct | return | await | select | string | sstring | raw_string | struct | switch | var)
  ...
  /* ********************************************************** */
  // !('!' | '&' | '(' | '*' | '+' | '-' | ';' | '<-' | '^' | 'type' | '{' | '|' | '|=' | '||' | '}' | break | case | char | const | continue | decimali | default | defer | else | fallthrough | float | floati | for | func | go | goto | hex | identifier | if | int | interface | map | oct | return | await | select | string | sstring | raw_string | struct | switch | var)
  ...
  private static boolean VarSpec_1_0(PsiBuilder b, int l) {
    ...
    r = p && VarSpec_1_0_2(b, l + 1) && r;
    exit_section_(b, l, m, r, p, null);
    return r || p;
  }

  // [ '=' ExpressionList ]
  private static boolean VarSpec_1_0_2(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "VarSpec_1_0_2")) return false;
    VarSpec_1_0_2_0(b, l + 1);
    return true;
  }

  // '=' ExpressionList
  private static boolean VarSpec_1_0_2_0(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "VarSpec_1_0_2_0")) return false;
    boolean r, p;
    Marker m = enter_section_(b, l, _NONE_);
